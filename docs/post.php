...
$idaction = mysqli_query($conn, $idcheck) or die(mysqli_error($conn));

if (mysqli_num_rows($idaction) > 0)
{
	// Exists

	while ($metarow = mysqli_fetch_array($idaction))
        {
                $metaid = $metarow["ID"];
...
if ($result)
{
	while ($row = mysqli_fetch_array($result))
		...
		$output = mysqli_query($conn, $mysql) or die(mysqli_error($conn));

		if ($output)
		{
        		while ($display = mysqli_fetch_array($output))
        		{
                		$m_id = $display["ID"];
                		$m_date = $display["date"];
